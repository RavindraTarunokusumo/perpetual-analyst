...
- `src/perpetual_analyst/web/`: local Flask dashboard (`app.py` factory + routes, `queries.py` read-only view-model builders, `actions.py` write actions + run thread); started by `analyst web`
- `src/perpetual_analyst/daily_run.py`: orchestrator entry point
- `src/perpetual_analyst/cli.py`: typer CLI (`analyst topic add SLUG`, `analyst source add`, `analyst run`, `analyst web`)
